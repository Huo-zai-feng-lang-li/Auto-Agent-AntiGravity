...
> **Release v1.0.31: 智能自愈版**
>
> - ✅ **第三方启动智能修复**：当通过无参方式（如各类启动器）启动 IDE 时，自动检测并执行一次性智能重启修复，无需人工干预。
> - ✅ **全量环境修复**：自动扫描并修复所有桌面、任务栏、启动菜单快捷方式。
> - ✅ **参数完美继承**：保留原有 GPU 优化参数，从此告别渲染问题。
> - ✅ **静默自愈机制**：每次启动自动检测注册表状态，IDE 更新重置也不怕。
...
虽然 v1.0.29+ 版本支持**全自动修复**，但如果你想手动检查或自定义启动参数，请参考以下说明：
...
插件会自动从你**最完整**的快捷方式中提取参数（如 GPU 优化参数），并将其同步到 Windows 注册表。这保证了你即使通过“右键打开”或“双击文件”启动，也能同时拥有 **CDP 连接能力** 和 **原有性能优化参数**。

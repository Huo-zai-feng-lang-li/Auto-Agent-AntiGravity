...
class Relauncher {
    ...
    for (const shortcutPath of possiblePaths) {
      if (fs.existsSync(shortcutPath)) {
        ...
        shortcuts.push({
          ...
          type: shortcutPath.includes("Start Menu")
            ? "startmenu"
            : shortcutPath.includes("Desktop")
              ? "desktop"
              : "taskbar",
          args: info.args,
          target: info.target,
  ...
  async _readWindowsShortcut(shortcutPath) {
    const scriptPath = path.join(os.tmpdir(), "auto_accept_read_shortcut.vbs");

    try {
      const vbsScript = `
Option Explicit
On Error Resume Next
Dim WshShell, lnk, args, target
Set WshShell = CreateObject("WScript.Shell")
Set lnk = WshShell.CreateShortcut("${shortcutPath.replace(/"/g, '""')}")
...
WScript.Echo "ARGS:" & lnk.Arguments
WScript.Echo "TARGET:" & lnk.TargetPath
`;
      fs.writeFileSync(scriptPath, vbsScript, "utf8");

      const result = execSync(`cscript //Nologo "${scriptPath}"`, {
        encoding: "utf8",
        timeout: 2000, // VBScript is fast, 2s is plenty
      });
  ...
  async _modifyWindowsShortcut(shortcutPath) {
    const scriptPath = path.join(
      os.tmpdir(),
      "auto_accept_modify_shortcut.vbs",
    );
...
Option Explicit
On Error Resume Next
Dim WshShell, lnk, args, target, newArgs, pattern, regex
Set WshShell = CreateObject("WScript.Shell")
Set lnk = WshShell.CreateShortcut("${shortcutPath.replace(/"/g, '""')}")

WScript.Echo "BEFORE_ARGS:" & lnk.Arguments
WScript.Echo "TARGET:" & lnk.TargetPath

args = lnk.Arguments
pattern = "--remote-debugging-port=\\d+"
...
If regex.Test(args) Then
    newArgs = regex.Replace(args, "--remote-debugging-port=${BASE_CDP_PORT}")
    If newArgs <> args Then
        lnk.Arguments = newArgs
        lnk.Save
        WScript.Echo "AFTER_ARGS:" & newArgs
        WScript.Echo "RESULT:UPDATED"
    Else
        WScript.Echo "RESULT:ALREADY_CORRECT"
    End If
Else
    newArgs = "--remote-debugging-port=${BASE_CDP_PORT} " & args
    lnk.Arguments = newArgs
    lnk.Save
    WScript.Echo "AFTER_ARGS:" & newArgs
    WScript.Echo "RESULT:MODIFIED"
End If
...
`;
      ...
      const rawResult = execSync(`cscript //Nologo "${scriptPath}"`, {
        encoding: "utf8",
        timeout: 5000,
      });
      ...
      if (result === "MODIFIED") {
        ...
        return {
          success: true,
          modified: true,
          message: `Modified: ${path.basename(shortcutPath)}`,
        };
        ...
        return {
          success: true,
          modified: true,
          message: `Updated port: ${path.basename(shortcutPath)}`,
        };
        ...
        return {
          success: true,
          modified: false,
          message: "Already configured with correct port",
        };
      } else {
...
`;
  ...
  async _modifyLinuxDesktop(desktopPath) {
    try {
      ...
      const userDesktopDir = path.join(
        os.homedir(),
        ".local",
        "share",
        "applications",
      ...
      return {
        success: true,
        modified: true,
        message: `Modified: ${path.basename(targetPath)}`,
      };
    } catch (e) {
...
`;
  ...
  async relaunchWithCDP() {
    ...
    if (shortcuts.length > 0) {
      ...
      this.log(
        `Found ${shortcuts.length} shortcuts. Using primary: ${primaryShortcut.path}`,
      );
    ...
    if (!exePath) {
      return {
        ...
        action: "error",
        message:
          "Fatal: No shortcuts found and could not determine IDE executable path.",
      };
    }
